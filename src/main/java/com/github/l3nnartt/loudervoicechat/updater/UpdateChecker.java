  ...
  public void check() {
    try {
      ...
      if (addonVersion < serverVersion) {
        LouderVoiceChat.getLogger(
            "Outdated version of LouderVoiceChat detected, restart your Game");
        File file = initFile();
        Runtime.getRuntime()
            .addShutdownHook(
                new Thread(
                    () ->
                        new FileDownloader(
                                "http://dl.lennartloesche.de/loudervoicechat/8/LouderVoiceChat.jar",
                                file)
                            .download()));
      } else {
        LouderVoiceChat.getLogger(
            "You run on the latest version of LouderVoiceChat (" + addonVersion + ")");
      }
    } catch (IOException e) {
      e.printStackTrace();
    }
